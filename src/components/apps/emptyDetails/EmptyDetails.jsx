...
const REPORTS = {
  ...
  // בתוך phone, לצד title ו־body:
image: phoneReportImage,
...
const ITEM_TYPES = [
  { type: "phone", Icon: Phone, special: true, label: REPORTS.phone.label },
...
const ITEM_SIZE = 70;
const FOLDER_WIDTH = 124;
const FOLDER_HEIGHT = 98;
const CLOUD_Y = 92;
const FALL_SPEED = 135; // פיקסלים לשנייה
const RESPAWN_DELAY = 550; // ms בין פריטים
const CLOUD_WIDTH = 150;
const CLOUD_MIN_X = 20;
...
const EmptyDetails = ({ onComplete }) => {
  ...
  const initialQueue = useRef(
    shuffle(
      Array.from({ length: TOTAL_ITEMS }, (_, index) =>
        ITEM_TYPES[index % ITEM_TYPES.length].type
      )
    )
  );

  const [queue, setQueue] = useState(() => [...initialQueue.current]);
...
const [isImageExpanded, setIsImageExpanded] = useState(false);
  ...
  const cloudXRef = useRef(0);
  const cloudDirectionRef = useRef(1);
  ...
  const spawnNext = useCallback(() => {
    ...
    const q = [...initialQueue.current];
    if (q.length === 0) return;

    spawnLockRef.current = true;

    const [nextType, ...rest] = q;
    initialQueue.current = rest;
    setQueue(rest);
    ...
    const cloudCenter = cloudXRef.current + CLOUD_WIDTH / 2;
    const x = Math.min(
      Math.max(
        cloudCenter - ITEM_SIZE / 2 + (Math.random() - 0.5) * 50,
        0
      ),
      maxX
    );
    ...
    setCurrentItem({
      id: `${nextType}-${Date.now()}-${Math.random()}`,
      type: meta.type,
      x,
      y: CLOUD_Y,
    });

    // מונע ספאון כפול גם אם React מריץ עדכונים שוב ב-StrictMode.
    setTimeout(() => {
      spawnLockRef.current = false;
    }, 0);
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      ...
      popup === null &&
      queue.length === 0 &&
      collected.length === TOTAL_ITEMS
    ) {
      setPopup("final");
    }

    return () => clearTimeout(spawnTimerRef.current);
  }, [currentItem, popup, queue.length, collected.length, spawnNext]);

  // ---- לולאת האנימציה: ענן זז + פריט אחד נופל + לכידה ----
  useEffect(() => {
    const step = (ts) => {
      ...
      // הענן נע מצד לצד בראש המגרש.
      const cloudTravel = Math.max(0, width - CLOUD_WIDTH - CLOUD_MIN_X * 2);
      if (cloudTravel > 0) {
        let nextCloudX =
          cloudXRef.current + cloudDirectionRef.current * 95 * dt;

        if (nextCloudX >= CLOUD_MIN_X + cloudTravel) {
          nextCloudX = CLOUD_MIN_X + cloudTravel;
          cloudDirectionRef.current = -1;
        } else if (nextCloudX <= CLOUD_MIN_X) {
          nextCloudX = CLOUD_MIN_X;
          cloudDirectionRef.current = 1;
        }

        cloudXRef.current = nextCloudX;
      }
      ...
      if (currentItemRef.current && popupRef.current === null) {
        setCurrentItem((item) => {
          ...
          // כל פריט יכול להיכנס ל-collected פעם אחת בלבד.
          ...
          // פספוס = הפריט נגמר. לא מחזירים אותו לענן ולא סופרים אותו שוב.
          if (newY > height) {
            // return null;
            // פספוס — אותו הפריט חוזר ליפול מהענן עד שייאסף
  const maxX = Math.max(0, width - ITEM_SIZE);
  const cloudCenter = cloudXRef.current + CLOUD_WIDTH / 2;

  const nextX = Math.min(
    Math.max(cloudCenter - ITEM_SIZE / 2, 0),
    maxX
  );
    ...
    <div dir="rtl" className="ed-wrapper">
      <div className="ed-board-column">
        ...
        >
          <div
            className="ed-cloud"
            style={{ left: cloudXRef.current, transform: "none" }}
          >
            <Cloud size={118} strokeWidth={1.5} fill="#ffffff" />
          </div>
        ...
        <div className="ed-overlay">
          <div className="ed-popup">
            ...
            <button
              ...
              aria-label={
                tooltipOpen ? "הסתרת תמונת ההסבר" : "הצגת תמונת ההסבר"
              }
              aria-expanded={tooltipOpen}
              aria-controls="ed-help-image"
            >
              <HelpCircle size={20} />
            </button>

            {tooltipOpen && (
  <div id="ed-help-image" className="ed-help-image">
    <button
      type="button"
      className="ed-help-image-button"
      onClick={() => setIsImageExpanded(true)}
      aria-label="הגדלת תמונת ההסבר"
      aria-haspopup="dialog"
    >
      <img
        src={REPORTS[popup].image}
        alt={`דוגמה עבור ${REPORTS[popup].label}`}
        draggable={false}
      />
    </button>
  </div>
  ...
  createPortal(
    <div
      ...
      onKeyDown={(event) => {
        ...
        // משאירים את הפוקוס בכפתור הסגירה בזמן ההגדלה
        if (event.key === "Tab") {
      ...
      role="dialog"
      aria-modal="true"
      aria-label="תמונת ההסבר מוגדלת"
    >
      <button
        type="button"
        className="ed-image-lightbox__close"
        onClick={() => setIsImageExpanded(false)}
